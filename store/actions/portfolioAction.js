...
export const getMarket = () => async dispatch => {
    ...
    const config = {
      ...
      try {
        const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=200&page=1&sparkline=false&price_change_percentage=24h%2C7d%2C30d&locale=en&precision=2')
        .then((res) => res.json())
        .then((data) => {
            data.forEach((item) => {
                market.push({
